...
- **Rearrangeable Web Terminal workspace** — in Expert mode the fixed panel/terminal split is now a docking workspace: drag any panel or the terminal card into side-by-side splits or tab stacks, and the "+" menu docks a new panel beside the active group. Your arrangement is saved per project and restored on reload, and "Reset layout" returns to the default. Simple mode stays a fixed, locked layout, and agent-driven panel changes still apply in either mode.
