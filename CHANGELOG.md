...
- **Two-persona multi-user demo** — the `control-assistant` preset now ships the full multi-user web-terminal stack out of the box: operator and physicist personas (new `control-assistant-operator` / `control-assistant-physicist` presets), an alice/bob roster, and the Bluesky bridge service. `osprey deploy up` (local mode) auto-renders any referenced persona project that hasn't been built yet, so one `osprey build` + `osprey deploy up` brings up the whole stack with no per-persona builds. A new multi-user demo walkthrough documents the flow, including the virtual-accelerator reconfiguration for real scans.
...
- `deploy_services: false` build-profile key marks an **attached project** — one that connects to services deployed by another OSPREY project on the same host instead of scaffolding its own (service sections still parse and validate, but the built project gets `deployed_services: []` and no `services/` tree; `osprey deploy up` in it is a clean no-op). The persona presets (`control-assistant-operator`/`-physicist`) now build attached, consuming the demo stack's shared services instead of carrying deployable copies whose host ports collide with it.
