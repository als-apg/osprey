...
- `osprey query` (and the SDK runner / dispatch worker) now expand `${VAR}` placeholders in a custom provider's `base_url` against the project `.env`, matching the other CLI commands. Previously the literal `${ARGO_PROD_URL}` reached the translation proxy and requests failed with `"${ARGO_PROD_URL}/v1/messages..." cannot be parsed as a URL`. Provider-spec resolution is now funnelled through a single `.env`-aware loader, and non-native (OpenAI-protocol) providers correctly start the translation proxy on the `query`/SDK and dispatch paths (#307).
