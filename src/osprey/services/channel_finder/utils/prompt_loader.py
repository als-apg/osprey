...
def _try_load_builtin_prompts(
    ...
    try:
        ...
        module_path = f"osprey.services.channel_finder.prompts.{pipeline_mode}"
        ...
        missing_attrs = [attr for attr in required_attrs if not hasattr(prompts_module, attr)]
        if missing_attrs:
            logger.warning(f"⚠ Built-in prompts missing required submodules: {missing_attrs}")
            return None

        return prompts_module

    except Exception as e:
        logger.debug(f"Failed to load built-in prompts for {pipeline_mode}: {e}")
        return None
