"""Full dependency healthcheck for the scan panels sidecar (task 1.4).
...
async def _probe_tiled_http(request: Request) -> dict[str, Any]:
    ...
    client: httpx.AsyncClient = request.app.state.client
    base = os.environ.get("SCAN_PANELS_TILED_URL", _DEFAULT_TILED_URL).rstrip("/")
...
async def _probe_va_tcp(_request: Request) -> dict[str, Any]:
    ...
    """
    start = time.monotonic()
    addr = os.environ.get("SCAN_PANELS_VA_ADDR", _DEFAULT_VA_ADDR)
