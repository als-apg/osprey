"""The scan panels sidecar's FastAPI app: skeleton wiring for the operator
...
from osprey.services.scan_panels import execute, read_proxy
from osprey.services.scan_panels import health as health_routes
...
def _resolve_bridge_url() -> str:
    ...
    Mirrors ``osprey.mcp_server.scan.server_context.ScanContext._resolve_bridge_url``
    so the sidecar and the scan MCP agree on which bridge instance to talk to.
...
app = FastAPI(title="OSPREY Scan Panels", lifespan=_lifespan)
