...
def deploy_up_web_terminals(
    ...
    # fully-reconciled state -- see _run_verify_script for why its result is
    # advisory only and never raises from here. The host-reachability probe
    # runs last and is likewise advisory (see _warn_if_web_stack_unreachable:
...
def _warn_if_web_stack_unreachable(config: dict, attempts: int = 5, delay: float = 2.0) -> None:
    ...
    Advisory like :func:`_run_verify_script`: the containers themselves are
