...
class ModelPreferencesManager:
    ...
    def _discover_openai_models(self, config_path: str) -> List[str]:
        ...
        try:
            ...
            api_config = config.get('api', {}).get('providers', {}).get('openai', {})
    ...
    def _discover_openai_compatible_models(self, config_path: str, provider_name: str) -> List[str]:
        ...
        try:
            ...
            # Skip if credentials are still missing after resolution
            if not api_key or not base_url:
                return []
            ...
            try:
                ...
                # Silently fail on API errors (auth, network, etc.)
                return []
            
        except Exception:
            # Silently fail and return empty list to fall back to static models
            return []
