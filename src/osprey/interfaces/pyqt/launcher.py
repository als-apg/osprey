...
def main():
    ...
    print("🚀 Launching Osprey Framework GUI...")
    ...
    try:
        ...
        config_path = None  # Let the framework find the config
        if len(sys.argv) > 1:
            ...
            if Path("config.yml").exists():
                ...
                print(f"✅ Found config.yml in current directory")
            else:
                print("⚠️  No config.yml found in current directory")
                print("   The GUI will attempt to use framework defaults")
                print("   You can specify a config file: osprey-gui /path/to/config.yml")
