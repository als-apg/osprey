...
class MessageHandlers:
    ...
    def __init__(self, event_bus: EventBus, conversation_id_provider: Callable[[], str | None]):
        """
    ...
    def on_message_received(self, message: str):
        ...
        if conversation_id:
            ...
            self.event_bus.publish(
                ...
                {"conversation_id": conversation_id, "message_type": "agent", "content": message},
            )

            # Publish event to update conversation list
            self.event_bus.publish(
                EventTypes.CONVERSATION_UPDATED, {"conversation_id": conversation_id}
            )
    ...
    def on_status_update(
        self, status: str, component: str = "base", model_info: dict | None = None
    ):
        """
        ...
        self.event_bus.publish(
            ...
            {"status": status, "component": component, "model_info": model_info or {}},
        )

        # Publish status bar update event
        self.event_bus.publish("update_status_bar", {"message": status})
    ...
    def on_error(self, error: str):
        ...
        """
        # Publish error event
        self.event_bus.publish(EventTypes.ERROR_OCCURRED, {"error": error})

        # Publish display error event
        self.event_bus.publish("display_error", {"error": error})

        # Publish status update
        self.event_bus.publish(
            EventTypes.STATUS_UPDATE,
            {"status": f"Error: {error}", "component": "error", "model_info": {}},
        )
    ...
    def on_llm_detail(self, detail: str, event_type: str = "base"):
        ...
        self.event_bus.publish(
            EventTypes.LLM_DETAIL,
            {"detail": detail, "event_type": event_type, "timestamp": timestamp},
        )
    ...
    def on_tool_usage(self, tool_name: str, reasoning: str):
        ...
        self.event_bus.publish(
            EventTypes.TOOL_USAGE,
            {"tool_name": tool_name, "reasoning": reasoning, "timestamp": timestamp},
        )
