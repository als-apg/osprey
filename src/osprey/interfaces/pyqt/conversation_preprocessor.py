...
class ConversationPreprocessor:
    ...
    def preprocess_message(
        ...
        is_asking_about_previous = any(
            [
                "when i asked" in message_lower
                and (
                    "earlier" in message_lower
                    or "before" in message_lower
                    or "question" in message_lower
                    or "this" in message_lower
                ),
                "what was" in message_lower
                and ("when" in message_lower or "earlier" in message_lower),
                "do you know what" in message_lower
                and ("when" in message_lower or "earlier" in message_lower),
        ...
        for i in range(len(messages) - 1):
            if (
                ...
                and messages[i + 1].type in response_types
            ):
        ...
        logger.info(
            f"Found previous exchange - User: '{previous_user_msg.content[:50]}...', Response: '{previous_response_msg.content[:50]}...'"
        )
