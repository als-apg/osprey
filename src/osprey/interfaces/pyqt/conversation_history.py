...
class ConversationHistory:
    ...
    def save_conversation_history(self):
        ...
        if storage_mode != "json":
            logger.debug(
                f"Skipping JSON save - using {storage_mode} storage mode (messages stored in database)"
            )
            return
    ...
    def load_conversation_history(self):
        ...
        if storage_mode != "json":
            logger.debug(
                f"Skipping JSON load - using {storage_mode} storage mode (messages stored in database)"
            )
            return
        ...
        try:
            ...
            if self.gui.conversation_manager.load_from_json(conversations_file):
                logger.info(
                    f"✅ Loaded {len(self.gui.conversation_manager.conversations)} conversation(s) with messages from JSON file"
                )
                # Debug: Log message counts for each conversation
                for thread_id, conv in self.gui.conversation_manager.conversations.items():
                    logger.info(
                        f"  📂 Conversation '{conv.name}' ({thread_id}): {len(conv.messages)} messages"
                    )
                    for i, msg in enumerate(conv.messages):
                        logger.debug(
                            f"    Message {i + 1}: type={msg.type}, content_length={len(msg.content)}"
                        )
            else:
    ...
    def load_conversation_list(self):
        ...
        try:
            ...
            # For PostgreSQL checkpointer (AsyncPostgresSaver)
                ...
                logger.info(
                    "PostgreSQL checkpointer detected - loading conversations from database"
                )
                        ...
                        if messages and len(messages) > 0:
                            ...
                            for msg in messages:
                                if hasattr(msg, "type") and msg.type == "human":
                                    first_user_msg = (
                                        msg.content[:50] if hasattr(msg, "content") else None
                                    )
                                    break
                            ...
                            conv_name = (
                                first_user_msg
                                if first_user_msg
                                else f"Conversation {len(self.gui.conversations) + 1}"
                            )
