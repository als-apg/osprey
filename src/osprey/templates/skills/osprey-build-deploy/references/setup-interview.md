...
# Every generated file (docker-compose.yml, .gitlab-ci.yml, .env.template)
# and `osprey deploy` itself are parameterized by these values.
#
...
> 3. **Scaffold deploy infrastructure**: tell me 'scaffold the deploy infra' and I'll generate `docker-compose.yml`, `.gitlab-ci.yml`, and `scripts/verify.sh` from the templates, parameterized by your config.
...
> 5. **First push and deploy**: `git push`, watch CI, trigger the release job, then on the server: `git pull && osprey deploy up`."
