  ...
  web_terminals:
    ...
    personas:                              # optional catalog — omit entirely for zero-migration behavior
      assistant:                           # persona name — matches users[].persona / default_persona
        ...
        build_profile: "profiles/assistant.yml"  # registry mode: committed profile path (local mode: personas/<persona>.yml, the delta beside this project's profile)
      analysis:
...
| `auth.oidc.client_id_env` | string | no (default `OSPREY_AUTH_OIDC_CLIENT_ID`) | Name of the env var holding the client id. A **variable name**, never the value — OIDC credentials live in `.env.auth` (the only file the sidecar reads them from), never in config |
...
| `build_profile` | path | see Notes | The source the persona's project is rendered from — consumed differently per mode. **Registry mode:** a path to the persona's committed build-profile YAML, fed as the positional profile to the one `.gitlab-ci.yml` build job generated per non-default persona (required for a non-default persona; lint ERROR if missing). **Local mode:** the persona's **delta**, `personas/<persona>.yml` — a relative path that must resolve to a *direct child* of the `personas/` directory of the profile this project was built from, which is exactly what `osprey profile new` emits and writes into this catalog. `deploy up`'s auto-render passes that file positionally to `osprey build`; the delta merges over its own profile root, which is what gives the persona the same data tree, `.env` secrets and convention artifacts as its host. A bundled preset name, an absolute path, a path outside `personas/`, or one nested deeper than a direct child is a lint ERROR and a hard deploy failure. Required in local mode for any persona whose `project_path` is not already rendered |
...
14. **Personas and `image_source`**:
    ...
    - In local mode, a persona's `build_profile` must be `personas/<persona>.yml` — a relative path resolving to a direct child of the `personas/` directory of the profile the deployed project was built from. A bundled preset name, an absolute path, a path outside `personas/`, or a more deeply nested one is an ERROR, checked regardless of whether `project_path` already exists. Lint and `deploy up` share one predicate for this, so a value that lints clean is a value the deploy accepts.
