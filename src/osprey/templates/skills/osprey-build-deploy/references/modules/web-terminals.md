...
With authentication on, one optional `.env` entry applies: an `OSPREY_AUTH_PW_<USER>` plaintext to seed a chosen password for that user (hashed into `.env.auth` at deploy time, never shipped to a container). In `oidc` mode the client id/secret — under the names `auth.oidc.client_id_env` / `client_secret_env` point at — belong in `.env.auth`, **not** `.env`: the sidecar receives that file through compose `env_file:` and reads its credentials from nowhere else. The generated password hashes and session secrets live in `.env.auth` too, which the deploy writes. Three ignore templates keep it out of images and history: the project `.gitignore` and `.dockerignore` (both of which also list `.env.production`), plus the auth sidecar's own build-context `.dockerignore`. Never commit `.env.auth` or `.env.production`.
