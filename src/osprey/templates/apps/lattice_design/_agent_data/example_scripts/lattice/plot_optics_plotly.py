...
- Using save_artifact() for create_interactive_plot tool, or
  fig.write_html() for execute tool
"""
...
fig.add_trace(
    go.Scatter(x=s_pos, y=beta_x, name="\u03b2<sub>x</sub>", line=dict(color="blue", width=1.5)),
    row=1,
    col=1,
)
fig.add_trace(
    go.Scatter(x=s_pos, y=beta_y, name="\u03b2<sub>y</sub>", line=dict(color="red", width=1.5)),
    row=1,
    col=1,
)
...
fig.add_trace(
    go.Scatter(x=s_pos, y=eta_x, name="\u03b7<sub>x</sub>", line=dict(color="green", width=1.5)),
    row=2,
    col=1,
)
...
# Option A: via create_interactive_plot tool (preferred — auto-registers artifact)
