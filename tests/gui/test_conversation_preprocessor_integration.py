    ...
    with patch("osprey.interfaces.pyqt.gui.Path") as mock_path:
        ...
        with patch("osprey.interfaces.pyqt.gui.OspreyGUI.initialize_framework"):
            with patch("osprey.interfaces.pyqt.gui.OspreyGUI.setup_ui"):
                ...
                gui.input_field.toPlainText.return_value = (
                    "What was the weather when I asked earlier?"
                )
                ...
                with patch.object(
                    ...
                    assert mock_preprocess.called, (
                        "ConversationPreprocessor.preprocess_message should be called"
                    )
                    ...
                    explanation_call = [
                        call
                        for call in gui._append_colored_message.call_args_list
                    ...
                    print(
                        "   - GUI.send_message() calls ConversationPreprocessor.preprocess_message()"
                    )
