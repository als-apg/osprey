...
VALID launch token — zero HTTP calls issued.

This is the authoritative in-tool guard (see ``bluesky/tools/launch.py``): the
...
call) and supplies a correct ``BLUESKY_LAUNCH_TOKEN`` is still refused. General
...
Do NOT relax this test. It is the load-bearing safety contract for the bluesky
launch write path (analogous to ``tests/e2e/test_query_write_refused_e2e.py`` for
``channel_write``).
"""
...
async def test_writes_disabled_refuses_launch_with_valid_token_and_zero_http_calls(
    ...
    A malicious or hook-bypassed caller holding a correct BLUESKY_LAUNCH_TOKEN
    must not be able to launch a run when this deployment has writes
    ...
    monkeypatch.setenv("BLUESKY_LAUNCH_TOKEN", "genuinely-valid-token")
...
async def test_writes_enabled_with_valid_token_does_reach_the_bridge(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        return_value=(200, {"id": "x", "status": "running", "launched_by": "draft"}),
    ) as m:
        await fn(draft_revision=7)
...
async def test_writes_enabled_true_but_token_unset_refused_client_side(tmp_path, monkeypatch):
    ...
    With no BLUESKY_LAUNCH_TOKEN configured for this MCP server, launch_run
    must still refuse — client-side, before any HTTP call — rather than
    sending a launch request with no credential at all.
    """
    monkeypatch.chdir(tmp_path)
    (tmp_path / "config.yml").write_text(yaml.dump({"control_system": {"writes_enabled": True}}))
    monkeypatch.delenv("BLUESKY_LAUNCH_TOKEN", raising=False)
