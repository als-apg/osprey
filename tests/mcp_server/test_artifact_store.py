...
class TestSaveArtifactInjection:
    ...
    async def test_save_artifact_available_in_namespace(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            result = await fn(
                code='save_artifact("# Test", title="From Python")',
                description="test artifact injection",
...
"""
    ...
    @pytest.mark.asyncio
    async def test_notebook_artifact_always_created(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            result = await fn(
                code="x = 42",
                description="no explicit artifacts",
                execution_mode="readonly",
            )

        data = json.loads(result)
        assert data["status"] == "success"
        # Auto-created notebook artifact
        assert "artifact_ids" in data
