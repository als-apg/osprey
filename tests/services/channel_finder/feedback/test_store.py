...
def test_record_and_retrieve_success(store):
    ...
    hints = store.get_hints("show me magnets", "ERF")
    assert len(hints) == 1
...
def test_different_facilities_are_isolated(store):
    ...
    als_hints = store.get_hints("show me magnets", "ERF")
...
def test_query_normalization(store, variant):
    ...
    hints = store.get_hints(variant, "ERF")
    assert len(hints) == 1
...
def test_deduplication_of_identical_selections(store):
    ...
    hints = store.get_hints("magnets", "ERF")
    assert len(hints) == 1
...
def test_deduplication_ignores_channel_count_difference(store):
    ...
    store.record_success(
        query="magnets", facility="ERF", selections={"system": "MAG"}, channel_count=10
    )
    store.record_success(
        query="magnets", facility="ERF", selections={"system": "MAG"}, channel_count=20
    )
...
def test_different_selections_are_not_deduplicated(store):
    ...
    store.record_success(
        query="magnets", facility="ERF", selections={"system": "QUAD"}, channel_count=20
    )
...
def test_successes_capped_at_max_entries(store):
    ...
    hints = store.get_hints("magnets", "ERF", max_hints=20)
    assert len(hints) == 10
...
def test_failures_capped_at_max_entries(store):
    ...
    key = FeedbackStore._make_key("magnets", "ERF")
...
def test_file_persistence(tmp_path):
    ...
    store1.record_success(
        query="magnets", facility="ERF", selections={"system": "MAG"}, channel_count=42
    )
    del store1  # Let GC drop it

    store2 = FeedbackStore(path)
    hints = store2.get_hints("magnets", "ERF")
    assert len(hints) == 1
    assert hints[0]["channel_count"] == 42
...
def test_file_format_on_disk(store):
    store.record_success(
        query="magnets", facility="ERF", selections={"system": "MAG"}, channel_count=7
    )
...
def test_clear_wipes_all_data(store):
    ...
    store.record_failure(query="bpms", facility="ERF", partial_selections={}, reason="not found")

    store.clear()

    assert store.get_hints("magnets", "ERF") == []
    assert store.get_hints("bpms", "ERF") == []
...
def test_get_hints_most_recent_first(store):
    ...
    hints = store.get_hints("magnets", "ERF", max_hints=3)
    assert len(hints) == 3
...
def test_record_failure(store):
    ...
    hints = store.get_hints("broken query", "ERF")
    assert hints == []

    # But they are stored in the raw data
    raw = json.loads(store._path.read_text())
    key = FeedbackStore._make_key("broken query", "ERF")
...
def test_record_multiple_failures(store):
    ...
    raw = json.loads(store._path.read_text())
    key = FeedbackStore._make_key("bad query", "ERF")
...
def test_empty_store_returns_empty_hints(store):
    assert store.get_hints("anything", "ERF") == []


def test_empty_store_for_unknown_facility(store):
    store.record_success(
        query="magnets", facility="ERF", selections={"system": "MAG"}, channel_count=5
    )
...
def test_empty_store_for_unknown_query(store):
    ...
    assert store.get_hints("totally different query", "ERF") == []
...
def test_make_key_is_deterministic():
    key1 = FeedbackStore._make_key("show me magnets", "ERF")
    key2 = FeedbackStore._make_key("show me magnets", "ERF")
    assert key1 == key2
    assert len(key1) == 64  # SHA-256 hex digest length


def test_make_key_differs_for_different_inputs():
    key_a = FeedbackStore._make_key("query a", "ERF")
    key_b = FeedbackStore._make_key("query b", "ERF")
    assert key_a != key_b
...
def test_store_created_in_nonexistent_subdirectory(tmp_path):
    ...
    store.record_success(query="test", facility="ERF", selections={"x": "y"}, channel_count=1)

    assert deep_path.exists()
    hints = store.get_hints("test", "ERF")
    assert len(hints) == 1
...
def test_list_keys_returns_metadata(store):
    store.record_success(
        query="show me magnets", facility="ERF", selections={"system": "MAG"}, channel_count=10
    )
    store.record_failure(
        query="show me magnets",
        facility="ERF",
...
def test_update_record_failure(store):
    store.record_failure(
        query="bad", facility="ERF", partial_selections={"system": "MAG"}, reason="no options"
    )
    key = FeedbackStore._make_key("bad", "ERF")
    entry = store.get_entry(key)
...
def test_export_data_returns_complete_store(store):
    ...
    store.record_failure(query="bad", facility="ERF", partial_selections={}, reason="fail")
...
def test_meta_populated_on_new_records(store):
    ...
    store.record_success(
        query="Show Me Magnets", facility="ERF", selections={"system": "MAG"}, channel_count=5
    )
    key = FeedbackStore._make_key("Show Me Magnets", "ERF")
    entry = store.get_entry(key)
    assert entry["_meta"]["query"] == "Show Me Magnets"
    assert entry["_meta"]["facility"] == "ERF"
...
def test_search_by_keywords_single_match(store):
    ...
    store.record_success(
        query="show me magnets", facility="ERF", selections={"system": "MAG"}, channel_count=42
    )
    store.record_success(
        query="find BPM positions", facility="ERF", selections={"system": "BPM"}, channel_count=10
    )
