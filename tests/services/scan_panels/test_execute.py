"""Unit tests for the scan panels sidecar's execute route (task 1.3).

Exercises ``osprey.services.scan_panels.execute.router`` mounted onto a local
...
from osprey.services.scan_panels import execute
...
def test_execute_armed_via_config_token(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    # No env var token -- resolved from scan.promote_token in config.yml instead.
