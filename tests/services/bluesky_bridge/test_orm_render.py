...
def _rows(
    ...
    *,
    ...
    current (idle ones read back 0.0) alongside every readback -- the document
    ...
    for j, corrector in enumerate(correctors):
        for current in currents:
            ...
            for i, readback in enumerate(readbacks):
                row[readback] = float(truth[i, j] * current)
            rows.append(row)
...
def test_panel_order_leads_with_the_fit_and_demotes_the_sweeps() -> None:
    ...
    figure = orm.render(rows, _params(correctors=correctors, readbacks=readbacks))
...
def test_response_by_bpm_carries_only_fitted_correctors() -> None:
    ...
    figure = orm.render(rows, _params(correctors=correctors, readbacks=readbacks, num=num))
