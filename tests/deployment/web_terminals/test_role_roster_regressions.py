...
unprivileged ``ariel`` tier shared with the roster, and a ``default_persona``
...
class TestRoleOnlyRosterIsIndistinguishable:
    ...
    def test_the_session_roles_are_exactly_the_roster_bindings(
        ...
        user's role — including ``ariel``, whose card is shared and whose role
        is therefore the opener's rather than its own, but still rendered,
        because the roster the sidecar keys this table from lists it.
        """
        # Arrange
        role_bound = _deploy_config(_role_bound(exemplar_web_terminals))
