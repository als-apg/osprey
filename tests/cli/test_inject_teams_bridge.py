"""Tests for the ``_inject_teams_bridge`` build step in ``osprey.cli.build_cmd``.

Covers the three responsibilities of the teams-bridge-injection step: copying
the bundled ``templates/services/teams_bridge/`` compose template, writing the
``services.teams_bridge`` config + registering it in ``deployed_services``
...
The compose template reads ``services.teams_bridge.trigger`` with no ``|
...
Mirrors ``test_inject_gchat_bridge.py``, the other queue-fed chat channel, which
the injector is a structural copy of. Three things genuinely differ. The
credential group is the Azure one, and it is five variables rather than three.
The template directory is the only bundled one with a NESTED directory — the
Azure Functions relay that fills the queue ships beside the compose file,
because the operator publishes it out of the deployment repo — so the copy is
asserted recursively, and asserted to leave a source checkout's byte-compiled
residue behind. And there is no bind mount at all: a Service Bus connection
string is a variable, not a key file, so the state volume stands alone.
"""
...
from osprey.cli.build_cmd import _inject_teams_bridge, build
from osprey.cli.build_injectors import _refresh_service_dir
...
#: The five credentials the operator supplies by hand. Each stays a BARE
#: ``${VAR}``: the bridge's own ``require_startup()`` is what fails closed on
#: them, naming every missing one at once.
_TEAMS_CREDENTIALS = (
    "TEAMS_APP_ID",
    "TEAMS_APP_SECRET",
    "TEAMS_TENANT_ID",
...
def _render_bridge_compose(project_path: Path) -> str:
    ...
    template = env.get_template("services/teams_bridge/docker-compose.yml.j2")
...
def _build_bridge_project(
    runner: CliRunner, tmp_path: Path, *, teams_bridge: str = "teams_bridge: {}"
) -> tuple[Path, object]:
    ...
    (repo_dir / "profile.yml").write_text(
        _PROFILE_YAML.format(teams_bridge=teams_bridge), encoding="utf-8"
    )
...
def test_inject_teams_bridge_copies_template_dir(tmp_path: Path) -> None:
    """The bundled compose template dir is copied into services/teams_bridge."""
    project_path = tmp_path / "project"
    project_path.mkdir()
    _write_config(project_path)

    _inject_teams_bridge(TeamsBridgeProfileConfig(), project_path=project_path)

    dest = project_path / "services" / "teams_bridge"
    assert (dest / "docker-compose.yml.j2").is_file()
    assert (dest / "Dockerfile").is_file()
    assert (dest / ".dockerignore").is_file()


def test_inject_teams_bridge_copies_the_relay_source_beside_the_template(tmp_path: Path) -> None:
    """The nested relay directory travels with the template, file for file.

    Every other bundled service template is flat, so this is the one copy whose
    recursion matters. The relay is what fills the Service Bus queue the bridge
    drains: an operator publishes it to Azure Functions *out of the deployment
    repo*, which is the only reason its source is in a compose template
    directory at all. A copy that flattened or skipped it would leave a bridge
    that comes up healthy and never receives a message.
    """
    project_path = tmp_path / "project"
    project_path.mkdir()
    _write_config(project_path)
    ...
    relay = project_path / "services" / "teams_bridge" / "relay"
    for name in ("function_app.py", "validation.py", "host.json", "requirements.txt"):
        assert (relay / name).is_file(), f"the relay's {name} did not reach the project"


def test_inject_teams_bridge_leaves_byte_compiled_residue_behind(tmp_path: Path) -> None:
    """A ``__pycache__`` in the packaged template never reaches a project.

    The relay source is importable Python that the framework's own tests load by
    path, so in a source checkout the packaged template directory grows a
    ``__pycache__/`` as a side effect of running them. The recursive copy must
    not stamp one developer's byte-compiled artifacts into every project built
    from that checkout — they are machine- and interpreter-specific, and nothing
    in the deployment repo should carry an untracked build artifact whose origin
    the operator cannot explain.

    Exercised through :func:`_refresh_service_dir` on a synthetic template so
    the proof does not depend on whether this particular checkout happens to
    have run the relay tests yet.
    """
    src_dir = tmp_path / "packaged"
    (src_dir / "relay" / "__pycache__").mkdir(parents=True)
    (src_dir / "docker-compose.yml.j2").write_text("# template\n", encoding="utf-8")
    (src_dir / "relay" / "validation.py").write_text("x = 1\n", encoding="utf-8")
    (src_dir / "relay" / "__pycache__" / "validation.pyc").write_bytes(b"\x00")

    dest_dir = tmp_path / "project" / "services" / "teams_bridge"
    dest_dir.parent.mkdir(parents=True)

    assert _refresh_service_dir(src_dir, dest_dir, "teams_bridge", set()) is True

    assert (dest_dir / "relay" / "validation.py").is_file()
    assert not list(dest_dir.rglob("__pycache__")), "byte-compiled residue reached the project"


def test_inject_teams_bridge_writes_service_config(tmp_path: Path) -> None:
    """services.teams_bridge is written with path + trigger, and
...
def test_inject_teams_bridge_writes_custom_trigger(tmp_path: Path) -> None:
    ...
    _inject_teams_bridge(
        TeamsBridgeProfileConfig(trigger="other-trigger"), project_path=project_path
    )

    assert _read_config(project_path)["services"]["teams_bridge"]["trigger"] == "other-trigger"


def test_inject_teams_bridge_deployed_services_idempotent(tmp_path: Path) -> None:
    ...
    config = _read_config(project_path)
    deployed = [str(s) for s in config["deployed_services"]]
    assert deployed.count("teams_bridge") == 1
    # And the second run left one well-formed service block, not a merged mess.
    assert config["services"]["teams_bridge"]["path"] == "./services/teams_bridge"


def test_inject_teams_bridge_missing_config_yml_is_noop(tmp_path: Path) -> None:
    ...
    assert (project_path / "services" / "teams_bridge" / "Dockerfile").is_file()


def test_inject_teams_bridge_ships_the_partials_its_template_imports(tmp_path: Path) -> None:
    """The injector lands the shared macro files the compose template imports.

    The template imports ``services/_network_axis.j2`` and
    ``services/_env_axis.j2`` by paths relative to the PROJECT root, which is
    where the deploy-time renderer's loader is rooted. A project that received
    the template but not the partials therefore renders nothing at all:
    ``osprey up`` fails with ``TemplateNotFound``, one deploy too late. The
    injector runs alone here, with no build behind it, because that is exactly
    the path on which the partials would otherwise never arrive.
    """
    project_path = tmp_path / "project"
    project_path.mkdir()
    ...
    for partial in ("_network_axis.j2", "_env_axis.j2"):
        assert (project_path / "services" / partial).is_file(), (
            f"the compose template's own import target {partial} must travel with it"
        )


def test_inject_teams_bridge_skips_user_owned_service(tmp_path: Path) -> None:
    """A `scaffold claim services/teams_bridge` leaves the project copy untouched.
        ...
        yaml_rt.dump(
            {
                ...
                "services": {},
                "system": {"timezone": "UTC"},
                "scaffold": {"user_owned": ["services/teams_bridge"]},
            },
            fh,
        )
    dest = project_path / "services" / "teams_bridge"
    ...
    assert "teams_bridge" in [str(s) for s in _read_config(project_path)["deployed_services"]]


def test_inject_teams_bridge_renders_without_dispatch_pair_deployed(tmp_path: Path) -> None:
    ...
    # a raising handle_event leaves the queue message unsettled, the broker would
    # redeliver the same activity until it dead-letters. The `:?` guard is what
    ...
    assert env["DISPATCH_TRIGGER"] == "teams-question"
...
def test_full_build_with_bridge_renders_service_dir(runner: CliRunner, tmp_path: Path) -> None:
    ...
    assert (dest / "docker-compose.yml.j2").is_file()
    assert (dest / "Dockerfile").is_file()
    assert (dest / "relay" / "function_app.py").is_file()
    ...
    assert "teams_bridge" in deployed
    # The bridge is injected after the dispatch pair, so the pair the template
    # gates on is already registered by the time the bridge's block is written.
...
def test_full_build_compose_renders_with_no_unrendered_jinja(
    ...
    it renders an empty value (or, for `services.teams_bridge`, an undefined
    ...
    svc = yaml.safe_load(rendered)["services"]["teams-bridge"]
    ...
    assert svc["image"] == "${OSPREY_TEAMS_BRIDGE_IMAGE:-tmproj-teams-bridge:local}"
    assert svc["container_name"] == "tmproj-teams-bridge"
    assert svc["build"]["context"] == "./build/services/teams_bridge"
    assert svc["build"]["args"]["OSPREY_PROJECT_NAME"] == "tmproj"
    assert svc["command"] == ["python", "-m", "osprey.bridges.teams"]
    env = svc["environment"]
    ...
    # Azure credentials stay BARE — deliberately, and for a different reason than
    ...
    # which would boot the bridge authenticating as nobody or draining some other
    ...
    for var in ("EVENT_DISPATCHER_TOKEN", "DISPATCH_WORKER_TOKEN"):
        assert env[var] == f"${{{var}}}"
    assert env["TZ"] == "UTC"
    # A queue consumer opens no listening socket: no published ports, and no
    # healthcheck to probe one with.
    assert "ports" not in svc
    assert "healthcheck" not in svc
    # The state volume stands alone. Unlike the Google Chat bridge there is no
    # credential FILE to bind-mount: every Azure secret is a variable, so the
    # only host-side object this service needs is its own named volume.
    assert svc["volumes"] == ["teams_bridge_data:/data"]
...
def test_full_build_custom_trigger_reaches_rendered_compose(
    ...
    project_path, result = _build_bridge_project(
        runner, tmp_path, teams_bridge="teams_bridge:\n  trigger: other-trigger"
    )
    assert result.exit_code == 0, result.output

    svc = yaml.safe_load(_render_bridge_compose(project_path))["services"]["teams-bridge"]
...
def test_full_build_without_bridge_scaffolds_no_bridge_service(
    ...
    """The block is opt-in: a profile without it deploys no bridge (the step is skipped)."""
    project_path, result = _build_bridge_project(runner, tmp_path, teams_bridge="")
    assert result.exit_code == 0, result.output

    config = yaml.safe_load((project_path / "config.yml").read_text(encoding="utf-8"))
    assert "teams_bridge" not in config["deployed_services"]
    assert "teams_bridge" not in config.get("services", {})
    assert not (project_path / "services" / "teams_bridge").exists()


def test_full_build_all_three_bridges_coexist(runner: CliRunner, tmp_path: Path) -> None:
    """Declaring all three chat bridges deploys all three, each with its own trigger.

    The channels are independent injectors writing disjoint config keys, so a
    project may answer on Talk, Chat and Teams at once. Guards against a
    copy-paste slip in any injector clobbering another's service block — the
    likeliest defect in a family of near-identical functions.
    """
    repo_dir = tmp_path / "tmproj"
    repo_dir.mkdir()
    (repo_dir / "data").mkdir(exist_ok=True)
    (repo_dir / "triggers.yml").write_text(
        ...
        "  - name: gchat-question\n    source: webhook\n"
        "    action:\n      prompt: Answer on Chat.\n",
        encoding="utf-8",
    )
    (repo_dir / "profile.yml").write_text(
        _PROFILE_YAML.format(teams_bridge="teams_bridge: {}")
        + "nextcloud_bridge: {}\ngchat_bridge: {}\n",
        encoding="utf-8",
    )
    result = runner.invoke(
    ...
    config = yaml.safe_load((project_path / "config.yml").read_text(encoding="utf-8"))
    assert config["services"]["teams_bridge"]["trigger"] == "teams-question"
    ...
    deployed = config["deployed_services"]
    for name in ("teams_bridge", "gchat_bridge", "nextcloud_bridge"):
        assert name in deployed
        assert (project_path / "services" / name / "Dockerfile").is_file()
...
def test_full_build_bridge_without_dispatch_block_aborts(
    ...
    assert "teams_bridge requires a 'dispatch:' block" in errors
    assert not (repo_dir / "build" / "services" / "teams_bridge").exists()
...
def test_full_build_unknown_trigger_aborts(
    ...
    errors = _build_errors(caplog)
    assert "Build profile validation failed" in errors
    assert "teams_bridge.trigger 'ghost' is not declared in" in errors
    # The declared alternatives are named so the operator can fix it in one pass.
    assert "teams-question" in errors
    assert not (project_path / "services" / "teams_bridge").exists()
