...
The three ``control-assistant`` digests were re-recorded when the
bluesky-operating skill was renamed to ``operating-bluesky-plans``: the
preset's ``skills`` list is resolved content, so a rebuilt project really does
get a differently-named skill directory and really should see the staleness
advisory.
"""
