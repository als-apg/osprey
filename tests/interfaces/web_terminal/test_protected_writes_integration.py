...
def _baked_project(tmp_path_factory) -> Path:
    ...
    # The control-assistant preset ships both of these surfaces OFF: they are
    # administration, and the admin tier is what turns them back on. That is a
    # different, coarser boundary than the one under test -- a disabled panel
    # refuses every verb before there is a key to judge, so it would hide the
    # protected-key gate rather than exercise it (see `_require_config_panel`).
    # This suite is the admin tier's view, where the panel is open and the
    # protected set is the only thing standing.
    config_update_fields(
        project_dir / "config.yml",
        {"web.config_panel.enabled": True, "web.scaffold_gallery.write_enabled": True},
    )
