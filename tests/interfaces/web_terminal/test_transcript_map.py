...
class TestAKeyAnswersItselfByDefault:
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_a_stored_entry_does_not_answer_for_another_key(self):
        app = _app()
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_an_entry_naming_the_key_is_not_stored(self):
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_a_blank_transcript_id_is_ignored(self):
...
class TestAMoveSurvivesARestart:
    @pytest.mark.usefixtures("shared_root")
    def test_set_then_a_new_app_gets_the_mapped_id(self):
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_a_new_app_loads_lazily_without_a_lifespan(self):
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_clearing_an_entry_survives_too(self):
        app = _app()
        transcript_map.set(app, KEY, TRANSCRIPT)
        transcript_map.set(app, KEY, KEY)
...
class TestTheFileSitsBesideTheRecord:
    @pytest.mark.usefixtures("shared_root")
    def test_the_path_is_the_records_directory(self):
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_the_document_is_key_to_transcript_id(self):
        transcript_map.set(_app(), KEY, TRANSCRIPT)
    ...
    @pytest.mark.usefixtures("shared_root")
    def test_entries_that_cannot_name_a_transcript_are_dropped(self):
...
class TestAMapWithNoLocationStillAnswers:
    ...
    def test_set_and_get_work_in_memory(self):
        app = _app()
        transcript_map.set(app, KEY, TRANSCRIPT)
