...
virtual_accelerator:
  ...
  port: 5064
  # A second copy of the simulator, wired in as this deployment's `live`
  # target with a small fixed offset on its readouts, so `control_target_set
  # live` rehearses the real go-live ritual — probe, acknowledgment, strict
  # limits, approval prompts, LIVE MACHINE (stand-in) banner — on `osprey up`,
  ...
  # Going live for real is three steps: delete the line, point
  # `control_system.connector.epics.gateways` in your config.yml at your
  # facility, and replace
  # `control_system.target_switch.live_gateway_acknowledged` (the build wrote
  # the stand-in's loopback address there) with your own live gateway's
  # hostname.
