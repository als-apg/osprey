...
web_panels:
  ...
  - bluesky    # Scan queue + the selected run's live results
...
# that serves the two web_panels above. Each top-level block below is an
...
# the `web.panels.<id>` URLs. Remove this section (and the two panel ids above)
# to ship a Bluesky-free deployment.
bluesky:
  port: 8090
...
config:
  ...
  # SCAN panels — the operator UI for the mediated Bluesky stack (plan authoring,
  # the scan queue, and the selected run's live results), served by the
  # bluesky-panels sidecar.
...
render-build:
  ...
  before_script:
    ...
    - pip install --no-cache-dir "osprey-framework>=2026.8.0"
  script:
    - osprey validate
