...
| **config-key** | "`services.jupyter.containers.read.port_host`" | Two parts: the key parses, and the code reads it. Grep the loader and accessor for the path; a key nothing reads is dead documentation. `src/osprey/profiles/config_key_manifest.yml` is the authoritative list of live and deleted keys. | static + probe |
